...
let set_user_id () = 
  ...
  | None ->
    ...
    if Order_book_lib.User.get_balance user_id = 0.0 then begin
      ignore (Order_book_lib.User.create_user user_id 1000.0);
...
let initialize_system () =
  ...
  ignore (Order_book_lib.User.create_user 1 1000.0);
